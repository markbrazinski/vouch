...
def test_a_case_resumes_after_the_process_is_destroyed(durable):
    ...
    initial = first.evaluate_lot("LOT-1007", documents=[{"raw": COA_AMBIGUOUS}])
    ...
    lot_version_before = durable["corpus"].version_of("lot", "LOT-1007")
    ...
    assert durable["corpus"].lot("LOT-1007").status == "RELEASED"
    assert durable["corpus"].version_of("lot", "LOT-1007") == lot_version_before + 1
    releases = [
        entry for entry in durable["capabilities"].ledger
        if entry["action"] == "release_lot" and entry["target_id"] == "LOT-1007"
    ]
...
def test_a_restart_after_the_mutation_cannot_replay_it(durable):
    ...
    version_after = durable["corpus"].version_of("lot", "LOT-1007")
    inventory_after = durable["corpus"].get("inventory", "LOT-1007").usable
    ...
    assert durable["corpus"].version_of("lot", "LOT-1007") == version_after
    assert durable["corpus"].get("inventory", "LOT-1007").usable is inventory_after
...
def test_resuming_an_unknown_record_is_a_typed_failure(durable):
    ...
    with pytest.raises(VouchFailure) as caught:
        workflow.supply_human_evidence(
            decision_record_id="DR-does-not-exist", lot_id="LOT-1007",
            raw=QA_RETEST, authority_source="PLANT-QA-LAB",
        )
...
def test_evaluate_lot_on_an_existing_record_id_continues_it(durable):
    ...
    resumed = _workflow(durable)
    again = resumed.evaluate_lot("LOT-1007", decision_record_id=record_id)
