...
The causal shape this pins, and it is easy to get backwards:
  ...
  * LOT-1004 and LOT-1005 hold 650 kg of the same material and have no
...
def test_frame_c_the_other_alloy_lots_do_not_rescue_c417(world):
    """LOT-1004 + LOT-1005 hold 650 kg of MAT-ALLOY-7 and no allocation row.
    ...
    undecided = sum(
        corpus.get("inventory", lot).quantity for lot in ("LOT-1004", "LOT-1005")
    )
...
def test_a_halt_before_the_snapshot_still_clears_on_reset():
    ...
    0 as "unknown, keep the finding" pinned LOT-1005 to SECURITY_HOLD through
...
def test_coverage_is_never_inferred_from_a_shared_material(world):
    ...
    LOT-1004 (450 kg) and LOT-1005 (200 kg) are MAT-ALLOY-7 and undecided. If
    ...
    assert not {"LOT-1004", "LOT-1005"} & {r.lot_id for r in rows}
    ...
    # C-419 has an EXPLICIT row (PC-2, LOT-1003), which is the point rather
    # than a counterexample: its 200 kg counts because someone allocated that
    # named lot to that order. LOT-1007 is also MAT-RESIN-3 and also undecided,
    ...
    assert [r.lot_id for r in resin_rows] == ["LOT-1003"]
    assert corpus.planned_coverage("C-419", "MAT-RESIN-3") == 200.0
    assert "LOT-1007" not in [r.lot_id for r in resin_rows]
...
def test_deferred_terminal_hold_policy_is_recorded_not_implemented():
    ...
    rather than fixed, because nothing exercises it: neither LOT-1004 nor
    LOT-1005 has an allocation row. This test exists so the gap is visible and
