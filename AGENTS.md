...
The repository has a large regression suite. Do NOT run the full suite after every change.

Use the smallest test tier appropriate to the work.

### Tier 1 — Inner Loop
Run after normal implementation edits.

- tests directly covering changed modules/components
- relevant unit/component tests
- target runtime: <60 seconds

Do not run the entire backend/frontend suite for copy, styling, bounded UI, or isolated logic changes.

### Tier 2 — Demo-Critical Feature Gate
Run before declaring a commission complete.

Must cover the canonical Vouch paths:
...
- Decision Workspace routing/source behavior
- DecisionRecord continuity

Target runtime: <5 minutes.

### Tier 3 — Full Regression
Run the full backend + frontend suite only at integration boundaries:

- shared/core workflow changes
- authority or mutation changes
- canonical fixture/data changes
- merge/integration
- deployment
- film/final freeze

Do not block normal iteration on Tier 3.

### Tier 4 — Live AWS Qualification
Run only when relevant AWS-dependent behavior changed, including:

- AgentCore/runtime
- IAM/BFF deployment behavior
- Guardrails
- Textract-dependent PDF bytes
- deployed API behavior

Do not rerun live qualification for unrelated frontend work.

### Reporting

When finishing a commission, report:

- Tier(s) run
- tests passed/failed
- whether a full regression is still pending

Do not claim the repository is fully regression-green unless Tier 3 actually ran.
...
Command: `make fast` (~8s; budget <60s).

- every unit/component/contract test
- excludes the deliberate-sleep concurrency probes, the real-vite-build
  test, and the real-poll-interval transport tests
...
Command: `make gate` (~4s; budget <5min).

### Tier 3 — Full Regression
Command: `make full` (~26s). Run only at integration boundaries:
...
### Tier 4 — Live AWS Qualification
Command: `make live` (needs `VOUCH_LIVE_AWS=1` + gatehouse credentials).
Run only when relevant AWS-dependent behavior changed, including:
